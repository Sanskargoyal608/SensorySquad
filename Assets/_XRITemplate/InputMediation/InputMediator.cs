    ...
    public static class InputMediator
    {
        ...
        abstract class ConsumeProcessor<TValue> : InputProcessor<TValue> where TValue : struct
        {
            ...
            public override TValue Process(TValue value, InputControl control)
            {
                ...
                var actionMatched = (currentState.m_LockedAction == actionIndex) || (currentState.m_AllowedAction1 == actionIndex) || (currentState.m_AllowedAction2 == actionIndex);
        ...
        {
            InputSystem.RegisterProcessor<ConsumeFloat>(nameof(ConsumeFloat));
            InputSystem.RegisterProcessor<ConsumeVector2>(nameof(ConsumeVector2));
            InputSystem.RegisterProcessor<ConsumeVector3>(nameof(ConsumeVector3));
            InputSystem.RegisterProcessor<ConsumeQuaternion>(nameof(ConsumeQuaternion));
        ...
        {
            s_Updating = true;
            var actionList = InputSystem.ListEnabledActions();
            ...
            for (var i = 0; i < bindingCount; i++)
            {
                ...
                if (!currentBinding.effectiveProcessors.Contains(k_ConsumeKey))
                {
                    ...
                    if (currentBinding.processors.Length > 0)
                        action.ApplyBindingOverride(i, new InputBinding { overrideProcessors = $"{bindingType}(actionIndex={actionIndex}), {currentBinding.processors}" });
                    else
                        action.ApplyBindingOverride(i, new InputBinding { overrideProcessors = $"{bindingType}(actionIndex={actionIndex})" });
                }
            }
        }
